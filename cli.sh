...
cmd_propose() {
  ...
  # ── Mode: edit existing rules (no file given, library has changes) ──
  if [[ -z "$file" ]]; then
    local changed
    changed=$(git -C "$LIBRARY_PATH" status --porcelain 2>/dev/null)

    if [[ -z "$changed" && -z "$message" ]]; then
      err "Usage:"
      err "  $(basename "$0") propose <file.mdc> [--category <cat>]   Add a new rule"
      err "  $(basename "$0") propose \"message\"                       PR for edits to existing rules"
      exit 1
    fi

    if [[ -z "$changed" ]]; then
      err "No changes detected in the library at $LIBRARY_PATH"
      info "Edit a symlinked rule first, then run this command."
      exit 1
    fi

    echo -e "${BOLD}Changed files:${NC}"
    echo ""
    git -C "$LIBRARY_PATH" status --short
    echo ""
    ...
    local branch
    branch="update/$(echo "$message" | tr '[:upper:]' '[:lower:]' | sed 's/[^a-z0-9]/-/g' | sed 's/--*/-/g' | sed 's/^-//;s/-$//' | cut -c1-50)"
...
Auto-generated by \`cli.sh propose\`"
  ...
  # ── Mode: propose a new rule from a local file ──
  require_jq
  ...
  git commit -m "Add rule: $category/${rule_id}" -m "$desc"
  git push -u origin "$branch"

  local pr_body
  pr_body="## New Rule Proposal
...
Auto-generated by \`cli.sh propose\`"
  ...
  _open_pr "Add rule: $category/${rule_id}" "$pr_body" "$branch"
...
cmd_help() {
  ...
  echo "  propose \"message\"           Propose edits to existing rules via PR"
  ...
  echo "  $script propose \"Fix typo in no-secret-commit\"               # edit rule"
  echo "  $script doctor"
  echo ""
