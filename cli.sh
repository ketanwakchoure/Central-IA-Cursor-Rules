...
cmd_sync() {
  ...
  if ! jq empty "$CONFIG_FILE" 2>/dev/null; then
    err "$CONFIG_FILE contains invalid JSON. Fix it and try again."
    exit 1
  fi
...
_add_item() {
  ...
  if [[ -n "$already" ]]; then
    warn "$(echo "$type" | awk '{print toupper(substr($0,1,1)) substr($0,2)}') '$item_id' is already in $CONFIG_FILE"
  else
    local tmp
    tmp=$(mktemp)
...
Auto-generated by \`cursor-rules propose\`"
  ...
  local item_type_label
  item_type_label=$(echo "$item_type" | awk '{print toupper(substr($0,1,1)) substr($0,2)}')
  local pr_body="## New ${item_type_label} Proposal
