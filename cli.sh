...
Auto-generated by \`cli.sh propose\`"
  ...
  if gh pr create \
    --title "Add rule: $category/${rule_id}" \
    --body "$pr_body" \
    ...
    echo ""
    warn "Could not auto-create PR. Push succeeded -- create the PR manually:"
...
cmd_help() {
  ...
  echo "  propose <file> [--category] Contribute a NEW local rule to the library via PR"
  echo "  pr [message]                Publish edits to existing rules via PR"
  ...
  echo "  $script pr \"Fix typo in no-secret-commit\"    # edit existing rule"
  echo "  $script doctor"
  echo ""
